...
<section class="page-banner">
    <div class="page-banner-content">
        <span class="page-badge">CENTRAL DE ATENDIMENTO</span>
        <h1>Chamados</h1>
        <p>Visualize, filtre e acompanhe todos os chamados cadastrados no sistema.</p>
    </div>
...
<section class="filter-panel">
    <form action="{{ route('tickets.index') }}" method="GET" class="filter-panel-grid">

        <div class="field field-search">
            <label for="search">Buscar chamado</label>
            <input
                type="text"
                name="search"
                id="search"
                value="{{ request('search') }}"
                placeholder="Digite protocolo ou título..."
            >
        </div>
        ...
        <div class="filter-buttons">
            ...
            <a href="{{ route('tickets.index') }}" class="btn-secondary">Limpar</a>
        </div>
    </form>
</section>
...
<section class="list-panel">
    <div class="list-panel-header">
        <div>
            ...
            <p>{{ $tickets->total() }} chamado(s) encontrado(s)</p>
        </div>
    </div>
    ...
    <div class="ticket-cards">
        @forelse($tickets as $ticket)
            <a href="{{ route('tickets.show', $ticket) }}" class="ticket-card">
                <div class="ticket-card-left">
                    <div class="ticket-card-icon">🎫</div>

                    <div class="ticket-card-info">
                        <span class="ticket-code">{{ $ticket->protocol }}</span>
                        <h3>{{ $ticket->title }}</h3>
                        <p>
        ...
        @empty
            <div class="empty-box">
                ...
                <p>Não há chamados cadastrados ou nenhum resultado para o filtro informado.</p>
                <a href="{{ route('tickets.create') }}" class="btn-primary">
